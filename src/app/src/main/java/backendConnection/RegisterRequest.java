...
public class RegisterRequest extends AsyncTask<User, Void, MyBean> {
    private static MyApi myApiService = null;
    private ApiListener apiListener;

    public RegisterRequest(ApiListener apiListener) {
    ...
    protected MyBean doInBackground(User... params) {
        ...
        User user = params[0];
        try {
            return myApiService.register(user.getEmail(), user.getPassword()).execute();
        } catch (IOException e) {
            e.getMessage();
        }
        return null;
